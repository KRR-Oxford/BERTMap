...
class DirectBertExperiment(DirectSearchExperiment):
    ...
    def __init__(self, src_onto_iri_abbr, tgt_onto_iri_abbr, 
                 src_data_tsv, tgt_data_tsv, task_suffix="small", 
                 bert_name="bio_clinical_bert", bert_path="emilyalsentzer/Bio_ClinicalBERT"):
        super().__init__(src_onto_iri_abbr, tgt_onto_iri_abbr, src_data_tsv, tgt_data_tsv, task_suffix=task_suffix, exp_name=bert_name)
        self.bert = PretrainedBert(bert_path)
    
    def lexicon_process(self, entity_lexicon, pooling_strategy="last-2-mean"):
        # strategy "last-2-mean" or "last-1-cls"
        entity_lexicon = entity_lexicon.replace(" <property> ", " ").replace(" <sep> ", " ")
        lexicon_embed = self.bert.get_basic_sent_embeddings(entity_lexicon, strategy=pooling_strategy)
