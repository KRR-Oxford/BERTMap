...
class NormEditSimMapping(OntoMapping):
    def __init__(self, src_ob: OntoBox, tgt_ob: OntoBox, candidate_limit: Optional[int] = 50, save_dir: str = ""):
    ...
    def align_one_class(self, from_class: ThingClass, flag: str) -> None:
        ...
        self.log_print(
            f"[Time: {round(time.time() - self.start_time)}][PID {os.getpid()}][{print_flag}][Class-idx: {from_class_idx}][Mapping: {result}]"
        )
