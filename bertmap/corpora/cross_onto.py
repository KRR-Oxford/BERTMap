...
class CrossOntoCorpus(OntoCorpus):
    def __init__(
        ...
        if type(src2tgt_mappings_file) is str:
    ...
    def splitting_corpus(
        ...
        sample_rate: int = 10,
    ) -> Tuple[CrossOntoCorpus, CrossOntoCorpus, CrossOntoCorpus]:
        ...
        train_maps, val_test_maps = train_test_split(
            OntoEvaluator.read_mappings(src2tgt_mappings_file), test_size=1 - train_ratio
        )
        val_maps, test_maps = train_test_split(
            val_test_maps, test_size=test_ratio / (val_ratio + test_ratio)
        )
        kwargs = {
            "src_onto_box": src_onto_box,
            "tgt_onto_box": tgt_onto_box,
    ...
    def cross_onto_synonyms(self) -> None:
    ...
    def cross_onto_nonsynonyms(self) -> None:
    ...
    def negative_sampling(
        self, cls_labels: List[str], map_idx: int, search_from: str = "SRC"
    ) -> None:
        ...
        """
        if search_from == "SRC":
            onto_text = self.src_ob.onto_text
        elif search_from == "TGT":
            onto_text = self.tgt_ob.onto_text
        else:
            raise TypeError
        for label in cls_labels:
            neg_idxs = [
                exclude_randrange(0, len(self.maps), exclude=map_idx)
    ...
    def generate_label_data(self, soft_neg_rate: int = 2) -> Optional[List]:
