...
class OntoAlignCorpora:
    ...
    def __init__(self, 
        ...
        if not from_saved:
            ...
            self.train_ss_co, self.val_ss_co, self.test_ss_co = \
                CrossOntoCorpus.splitting_corpus(src_ob, tgt_ob, src2tgt_mappings_file,
                                                 train_map_ratio, val_map_ratio, test_map_ratio, sample_rate)
    ...
    def from_saved(cls, save_dir) -> OntoAlignCorpora:
        ...
        with open(save_dir + "/corpora/info", "r") as f: lines = f.readlines()
        neg_rates = re.findall(r"<NegRate intra_onto_soft=([0-9]+) intra_onto_hard=([0-9]+) cross_onto_soft=([0-9]+)>", lines[3])[0]
    ...
    def semi_supervised_data(self, 
        ...
        random.shuffle(ss_train); random.shuffle(ss_train_with_ids) 
        random.shuffle(ss_co_val); random.shuffle(ss_co_test)
        ss_data = {
            "train": ss_train,
    ...
    def unsupervised_data(self, 
        ...
        random.shuffle(us_io_val); random.shuffle(us_val_with_ids)
