...
class OntoAlignCorpora:
    def __init__(
        ...
        if not from_saved:
            ...
            self.train_ss_co, self.val_ss_co, self.test_ss_co = CrossOntoCorpus.splitting_corpus(
                src_ob,
                tgt_ob,
                src2tgt_mappings_file,
                train_map_ratio,
                val_map_ratio,
                test_map_ratio,
                sample_rate,
    ...
    def __repr__(self):
        ...
        report += "\t<!-- NegRate refers to the number of (soft/hard) nonsynonym sampled for each synonym in a corpus -->\n"
    ...
    def from_saved(cls, save_dir) -> OntoAlignCorpora:
        ...
        oa_corpora.train_ss_co = CrossOntoCorpus(
            corpus_file=save_dir + "/corpora/co.corpus.ss.train.json"
        )
        oa_corpora.val_ss_co = CrossOntoCorpus(
            corpus_file=save_dir + "/corpora/co.corpus.ss.val.json"
        )
        oa_corpora.test_ss_co = CrossOntoCorpus(
            corpus_file=save_dir + "/corpora/co.corpus.ss.test.json"
        )
        ...
        neg_rates = re.findall(
            r"<NegRate intra_onto_soft=([0-9]+) intra_onto_hard=([0-9]+) cross_onto_soft=([0-9]+)>",
            lines[3],
        )[0]
        ...
        for line in lines:
            if re.findall(r"<SemiSupervisedSetting train=(.+) val=(.+) test=(.+)>", line):
                map_ratios = re.findall(
                    r"<SemiSupervisedSetting train=(.+) val=(.+) test=(.+)>", line
                )[0]
    ...
    def semi_supervised_data(
        ...
        report += (
            f"\tco_train: {len(ss_co_train)}, co_val: {len(ss_co_val)} co_test: {len(ss_co_test)}\n"
        )
