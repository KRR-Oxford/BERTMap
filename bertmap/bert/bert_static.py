...
class BERTStatic:
    ...
    def word_embeds(self, sents: List[str], neg_layer_num: int=-1):
        ...
        with torch.no_grad():
            ...
            batch_embeds = torch.mul(batch_embeds, mask)  # the masked positions are zeros nowl torch.mul is point-wise multiplication
    ...
    def sent_embeds_mean(self, sents: List[str], neg_layer_num: int=-1):
        ...
        batch_word_embeds, mask = self.word_embeds(sents, neg_layer_num=neg_layer_num)  # (batch_size, sent_len, hid_dim)
    ...
    def sent_embeds_cls(self, sents: List[str], neg_layer_num: int=-1):
        ...
        batch_word_embeds, _ = self.word_embeds(sents, neg_layer_num=neg_layer_num)  # (batch_size, sent_len, hid_dim)
        return batch_word_embeds[:, 0, :]  # (batch_size, hid_dim)
        
    def ontotext_embeds(self,  
                        classtexts_batch: Dict[str, Dict],
                        strategy: str="mean",
                        neg_layer_num: int=-1):
        ...
        for i in range(len(lens_batch)):
            start = end; end = lens_batch[i] + start
