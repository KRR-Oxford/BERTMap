...
for src, tgt in [("fma", "nci"), ("fma", "snomed"), ("snomed", "nci")]:
    ...
    src_onto.iri_lexicon_df(*properties).to_csv(f"{save_path}/{src}2{tgt}.small.labels.tsv", sep="\t", index=False)
    tgt_onto.iri_lexicon_df(*properties).to_csv(f"{save_path}/{tgt}2{src}.small.labels.tsv", sep="\t", index=False)
...
for name in ["fma", "nci", "snomed"]:
    ...
    onto.iri_lexicon_df(*properties).to_csv(f"{save_path}/{name}.whole.labels.tsv", sep="\t", index=False)
