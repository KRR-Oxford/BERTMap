...
data_base = "/home/yuahe/projects/BERTMap/data/largebio/ontos"
save_path= f"{data_base}/../labels"
properties = ["label"]
...
for src, tgt in [("fma", "nci"), ("fma", "snomed"), ("snomed", "nci")]:
    ...
    src_onto.create_class2text(*properties).to_csv(f"{save_path}/{src}2{tgt}.small.labels.tsv", sep="\t", index=False)
    tgt_onto.create_class2text(*properties).to_csv(f"{save_path}/{tgt}2{src}.small.labels.tsv", sep="\t", index=False)
...
for name in ["fma", "nci", "snomed"]:
    ...
    onto.create_class2text(*properties).to_csv(f"{save_path}/{name}.whole.labels.tsv", sep="\t", index=False)
