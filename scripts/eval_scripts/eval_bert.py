...
for name in ["mean", "cls"]:
    for src, tgt in [("fma", "nci"), ("fma", "snomed"), ("snomed", "nci")]:
        ...
        for threshold in [0.5, 0.6, 0.7, 0.8, 0.90, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99]:
            eval_results.append(pool.apply_async(OntoMapping.evaluate, args=(f"{dir_name}/{src}-{tgt}-combined.small.bc-{name}.tsv", 
                                                                                     ref_legal, ref_illegal, f"combined", threshold)))
            eval_results.append(pool.apply_async(OntoMapping.evaluate, args=(f"{dir_name}/{src}2{tgt}.small.bc-{name}.tsv", 
                                                                                     ref_legal, ref_illegal, f"{src}2{tgt}", threshold)))
            eval_results.append(pool.apply_async(OntoMapping.evaluate, args=(f"{dir_name}/{tgt}2{src}.small.bc-{name}.tsv", 
